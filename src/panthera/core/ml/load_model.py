...
def wrap_frozen_graph(
    ...
    try:
        ...
        raise ValueError(f"Tensor not found in graph. Verify your inputs {inputs} and outputs {outputs}.") from e
...
def load_frozen_graph(
    ...
    try:
        ...
        logger.error(f"Failed to parse the graph file. It may be corrupted or not a valid .pb file: {e}")
        raise
    except Exception as e:
        logger.error(f"Error reading the graph file {graph_path}: {e}")
        raise

    logger.debug(f"Successfully loaded GraphDef. Wrapping into ConcreteFunction with inputs={inputs}, outputs={outputs}")
