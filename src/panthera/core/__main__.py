...
@click.option("-m", "--model_type", default="modelp", 
...
@click.option("-f", "--fasta", type=str, required=True,
              help="Name of genomic fasta file.")
@click.option("-x", "--whatshap_extension", type=int, default=1000,
...
@click.option("-d", "--context_dist", type=int, default=3000, metavar="[50-15,000]",
...
@click.option("--gtf", type=str, default="genome/gencode.v46.basic.annotation.gtf",
              help="Directory and file name of GENCODE GTF")
@click.option("-r", "--ref_haplotypes_dir", type=str, default="genome/reference_haplotypes",
              help="Directory where reference haplotype VCFs are stored. If None, then "
                   "reference haplotype will be only GRCh38.")
@click.option("-b", "--genetic_background", default="NRG",
              type=click.Choice(['BASE', 'CUSTOM', 'ALL', 'NRG', 'SUB', 'AFR', 'AMR', 'EAS', 'EUR', 'SAS']),
...
@click.option("-g", "--gene_target", multiple=True, default=[],
...
@click.option("-k", "--custom_background", multiple=True, default=[],
...
@click.option("--write_full_output", is_flag=True,
...
@click.option("-s", "--batch_size", type=int, default=2, metavar="[1-512]",
...
@cli.command("isolate")
@click.option("-t", "--tsv", type=str, required=True,
...
@click.option("-f", "--fasta", type=str, required=True,
              help="Name of genomic fasta file.")
@click.option("-d", "--context_dist", type=int, default=3000, metavar="[50-10,000]",
              help="Length of sequence as context. A key factor affecting runtime. "
                   "Default of 3,000 refers to the distance of 1500 bp up- and downstream "
                   "from the first and last variant.")
@click.option("--gtf", type=str, default="genome/gencode.v46.basic.annotation.gtf",
...
@click.option("--specify_genomic_range", type=str, required=True,
...
@click.option("-a", "--add_mutation", type=str, default=None,
