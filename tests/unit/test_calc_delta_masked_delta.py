...
def masked_scorer() -> SSPScorer:
    ...
    scorer = SSPScorer(
        ...
        wt_seq="ATCGN", mt_seq="ATCGN",
        wt_acc=dummy_arr, wt_dnr=dummy_arr,
        mt_acc=dummy_arr, mt_dnr=dummy_arr
    )
...
def test_calc_masked_delta_integration(masked_scorer):
    ...
    mt_dnr = np.array([0.2, 0.5, 0.1], dtype=np.float32) # Diff: [0, 0, -0.8 (mask! 102p1 is unk)]
