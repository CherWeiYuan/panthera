...
def mock_dependencies():
    ...
    """
    with patch("panthera.core.ssp.onehotencoder.SeqEncoder") as MockEncoder, \
        ...
        instance.one_hot_encode.side_effect = lambda seq, schema: np.zeros((len(seq), 4))
...
def test_spliceai_predict_empty_seqs():
    ...
    acc, dnr = spliceai_predict([], batch_size=2, spliceai_model=mock_model)
    assert acc == []
    assert dnr == []
...
def test_spliceai_predict_valid_input(mock_dependencies):
    ...
    acc, dnr = spliceai_predict(seqs, batch_size=2, spliceai_model=mock_spliceai)
...
def test_spliceai_predict_batch_size_adjustment(mock_dependencies):
    ...
    spliceai_predict(seqs, batch_size=-5, spliceai_model=mock_model)
